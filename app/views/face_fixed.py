...
def detect_face_upload():
    ...
    if file and allowed_file(file.filename):
        try:
            ...
            # Use mock detection for reliability
...
def detect_face_camera():
    """Detect faces from camera image - Simplified and robust"""
    try:
        ...
        # Save detection to database (with error handling)
        try:
...
@login_required
def add_face():
    """Add a new known face - Simplified"""
    try:
        if 'image' not in request.files:
            return jsonify({'error': 'No image file provided'}), 400
        ...
        if file and allowed_file(file.filename):
            ...
            cv2.imwrite(os.path.join(current_app.config['UPLOAD_FOLDER']), filename)
            ...
            for face in faces[:1]:  # Use first face
                objects_detected.append({
                    'name': person_name,
                    'confidence': face['confidence'],
                    'box': face['box'],
